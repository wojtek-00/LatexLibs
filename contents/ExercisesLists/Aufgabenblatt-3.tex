...
\subsubsection{Geben Sie aus, wie viele Module in der Datenbank gespeichert sind.}
    ...
    \begin{sql}
        ...
        SELECT studiengang.name, count(distinct lehrende.pid) lehrendeCnt
            ...
            -- oder
        SELECT studiengang.name, COUNT(distinct person.pid) lehrendeCount
            FROM person, modul, modulzuordnung, studiengang
            WHERE studiengang.sid = modulzuordnung.sid
            AND modulzuordnung.mid = modul.mid
            AND modul.pid = person.pid
            GROUP BY studiengang.name
            ORDER BY COUNT(person.pid)
    \end{sql}
    ...
    \begin{sql}
        SELECT person.name
            ...
            GROUP BY person.name
            HAVING COUNT(veranstaltung.mid) = 1
    \end{sql}
